...
import './Connect4.css';
import './TicTacToe.css'; // Reuse the player-card and welcome-screen layouts

const QUICK_EMOJIS = ['😂', '😎', '😢', '😡', '👍', '🎉'];

const Connect4 = ({ onScoreUpdate, globalPlayerName, setGlobalPlayerName, onPlayMusic, onOpponentLeft, activeSocketRef }) => {
  const [socket, setSocket] = useState(null);
  const [phase, setPhase] = useState('nameInput'); // nameInput, waiting, playing, finished
  ...
  const getEmoji = (sym) => sym === 'Red' ? '🍎' : (sym === 'Yellow' ? '🥭' : '');
  ...
  useEffect(() => {
    ...
    const checkScoreUpdate = (state, currentSocketId) => {
      ...
      const mySymbol = state.players.Red?.id === currentSocketId ? 'Red' : (state.players.Yellow?.id === currentSocketId ? 'Yellow' : null);
    ...
    newSocket.on('gameUpdate', (state) => {
      ...
      setGameState(state);
      const mySymbol = state.players.Red?.id === newSocket.id ? 'Red' : 'Yellow';
      const oppSymbol = mySymbol === 'Red' ? 'Yellow' : 'Red';
    ...
    newSocket.on('gameStart', (state) => {
      ...
      let mySymbol = null;
      let oppName = '';
      if (state.players.Red.id === newSocket.id) {
        mySymbol = 'Red';
        oppName = state.players.Yellow.name;
      } else {
        mySymbol = 'Yellow';
        oppName = state.players.Red.name;
      }
      ...
      <div className="game-layout">
          ...
          <div className="game-info">
            ...
            <div className="player-card">
              ...
              <p className="player-turn">
                {gameState.currentPlayer === playerSymbol ? getEmoji(playerSymbol) : '⚫'}
              </p>
            </div>
            <div className="player-card opponent">
              <p className="player-name">Opponent</p>
              <p className="player-label">{opponentName}</p>
              <p className="player-symbol">{getEmoji(playerSymbol === 'Red' ? 'Yellow' : 'Red')}</p>
              <div className="player-stats" style={{ fontSize: '0.85rem', marginTop: '12px', padding: '8px', background: 'rgba(0,0,0,0.05)', borderRadius: '6px' }}>
                <div style={{ fontWeight: 'bold', marginBottom: '4px' }}>Stats</div>
                <div>🏆 Wins: {gameState.players[playerSymbol === 'Red' ? 'Yellow' : 'Red']?.score?.wins || 0}</div>
                <div>🤝 Draws: {gameState.players[playerSymbol === 'Red' ? 'Yellow' : 'Red']?.score?.draws || 0}</div>
                <div>❌ Losses: {gameState.players[playerSymbol === 'Red' ? 'Yellow' : 'Red']?.score?.losses || 0}</div>
                <div>🎮 Total: {gameState.players[playerSymbol === 'Red' ? 'Yellow' : 'Red']?.score?.total || 0}</div>
              </div>
              <p className="player-turn">
                {gameState.currentPlayer !== playerSymbol ? getEmoji(playerSymbol === 'Red' ? 'Yellow' : 'Red') : '⚫'}
              </p>
            </div>
          </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          ...
          {gameState.board.map((row, rIndex) => (
            <div key={rIndex} className="c4-row">
              {row.map((cell, cIndex) => (
                <div
                  key={cIndex}
                  className="c4-cell-wrapper"
                  onClick={() => handleColumnClick(cIndex)}
                >
                  <div className={`c4-cell ${cell ? cell.toLowerCase() : ''}`}>
                    {cell && <span className="c4-emoji">{getEmoji(cell)}</span>}
                  </div>
                </div>
              ))}
            </div>
        ...
        {gameState.winner && (
          <div className="reset-section">
            <button className="reset-btn" onClick={handlePlayAgain}>
              Play Again
            </button>
          </div>
        )}
          ...
          <div className="chat-messages">
            ...
            {isOpponentTyping && (
              <div className="chat-message opponent typing-indicator">
                <span className="chat-sender">{getEmoji(playerSymbol === 'Red' ? 'Yellow' : 'Red')}</span>
