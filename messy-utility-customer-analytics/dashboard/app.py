...
def main() -> None:
    ...
    st.dataframe(_load_csv("answer_1_abnormal_customers"), use_container_width=True)

    st.subheader("Regions with the highest electricity consumption")
    st.dataframe(_load_csv("answer_2_regions"), use_container_width=True)

    st.subheader("Customer categories with the largest consumption growth")
    st.dataframe(_load_csv("answer_3_category_growth"), use_container_width=True)

    st.subheader("Customers with missing or suspicious readings")
    st.dataframe(_load_csv("answer_4_reading_issues"), use_container_width=True)

    st.subheader("Bills that cannot be reconciled with meter consumption")
    st.dataframe(_load_csv("answer_5_unreconciled_bills"), use_container_width=True)

    st.subheader("Top 20 potentially anomalous customers")
    st.dataframe(_load_csv("answer_6_top_20_anomalous_customers"), use_container_width=True)
